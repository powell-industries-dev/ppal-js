...
const object = {
   ...
   */
  diff: function ({ original, updated, prefix = "" }) {
   ...
   */
  strip: function (obj, recursion = false) {
    // GUARD: Only accept objects
    if (!_.isObjectLike(obj) && recursion) {
    ...
    for (const key of _.keys(obj)) {
      ...
      // 1. Array → copy unchanged
      if (_.isArray(value)) {
        result[key] = _.cloneDeep(value); // shallow copy is fine, but _.cloneDeep keeps wrappers intact
        continue;
      }

      // 2. Plain object → maybe a wrapper, maybe nested
      if (_.isObjectLike(value)) {
        const innerKeys = _.keys(value);

        // 2a.  Wrapper object – a single `$…` key
        if (_.size(innerKeys) === 1 && _.startsWith(innerKeys[0], "$")) {
          result[key] = value[innerKeys[0]];
          continue;
        }

        // 2b.  Nested plain object – recurse
        result[key] = this.strip(value, true);
        continue;
      }

      // 3. Primitive, function, Date, RegExp, etc. → copy as‑is
      result[key] = value;
    }
...
const string = {
   ...
   */
  uniqueInArray: function ({ string, array }) {
    /** @type {Set} */
...
const date = {
   ...
   */
  format: function ({ date, type, timezone = "America/Denver" }) {
    switch (type) {
      case "DayOnly":
        /** @type { string } */
...
const behavior = {
   ...
   */
  inputDiff: function ({ value, initialData, theme }) {
